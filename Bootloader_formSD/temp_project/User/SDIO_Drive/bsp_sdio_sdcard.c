...
SD_Error SD_Init(void)
{
  ...
  SDIO_InitStructure.SDIO_ClockDiv = SDIO_TRANSFER_CLK_DIV;			//重新配置传输模式的时钟
...
/********************************************************************************************************/   
   ...
   * 没有响应  
	 */
  SDIO_CmdInitStructure.SDIO_Argument = 0x0;								//填充位
  SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_GO_IDLE_STATE;			//CMD0 命令
...
/********************************************************************************************************/
  ...
  if (errorstatus == SD_OK)	  	
  {
    ...
    CardType = SDIO_STD_CAPACITY_SD_CARD_V2_0;					//电压确认，SD卡版本为2.0
		
		/* 这个变量用作ACMD41的参数，用来询问是sdsc卡还是sdhc卡 */
    SDType = SD_HIGH_CAPACITY;										//卡容量类型
  }
...
SD_Error SD_InitializeCards(void)
{
  ...
  if (SDIO_GetPowerState() == SDIO_PowerState_OFF)
  {
	...
	/* 判断卡的类型 */
  if (SDIO_SECURE_DIGITAL_IO_CARD != CardType)			//如果使用大雨2GB的SD卡此时CardType已经为高容量的卡
  {
